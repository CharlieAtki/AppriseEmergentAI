...
class WorkspaceStreamService:
    """Live-dashboard boundary — initial snapshot; ORM never escapes.
    ...
    Ticket-minting (the pre-Centrifugo WS auth flow) used to live here too — it's
    gone along with the ticket-exchange auth model; auth is now the Centrifugo
    connect/subscribe proxy (see CentrifugoProxyService), which is why this class
    no longer needs a Redis dependency at all.
    """
