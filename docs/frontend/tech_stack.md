...
```ts
// invalidateQueries triggers a background refetch
queryClient.invalidateQueries({ queryKey: ['workspaces', workspaceId, 'tasks'] })

// setQueryData applies an immediate update with no round trip — use when the
// WebSocket event carries the complete new state
queryClient.setQueryData(
  ['workspaces', workspaceId, 'agents', agentId],
  (old: Agent | undefined) =>
    old ? { ...old, skills: applyDeltas(old.skills, skillDeltas) } : old
)
```
...
The Apprise backend publishes events to a Redis channel that the FastAPI WebSocket endpoint forwards to connected clients. The `useWorkspaceStream` hook maintains this connection for the full duration of the workspace session.
...
NEXT_PUBLIC_WS_URL=ws://localhost:8000       # WebSocket base URL (browser-accessible)
